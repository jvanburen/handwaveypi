...
class ImageBlob:
    ...
    def __str__(self):
        return "Blob at ({}, {}), with radius {}".format(
            round(self.center[0], 3),round(self.center[1], 3), round(self.radius, 3))
    def __repr__(self):
        return "<Blob object at ({}, {}), with radius {}>".format(self.center[0], self.center[1], self.radius)
    def pack_into(self, buffer, offset):
        type(self).STRUCT.pack_into(buffer, offset, self.center[0], self.center[1], self.radius)
